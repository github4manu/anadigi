...
public:
	ledFlasherClass(uint8_t pin, uint16_t on, uint16_t off, uint16_t longoff, uint8_t repeatof);
	void Update();
	void setOnOffTime(uint16_t on, uint16_t off, uint16_t longoff, uint8_t repeatof);
};
